...
export function App() {
    ...
    // 檢查內容是否包含 No enlarged/Boarderline enlarged/Enlarged 模式
    const hasEnlargedPattern = (content) => {
        if (!content) return false;
        // 匹配 "No enlarged/Boarderline enlarged/Enlarged" 模式（不區分大小寫，允許空格變化）
        const pattern = /No\s+enlarged\s*\/\s*Boarderline\s+enlarged\s*\/\s*Enlarged/i;
    ...
    const copyEnlarged = async (template, size) => {
        ...
        // 匹配 "No enlarged/Boarderline enlarged/Enlarged" 模式（不區分大小寫，允許空格變化）
        const pattern = /No\s+enlarged\s*\/\s*Boarderline\s+enlarged\s*\/\s*Enlarged/gi;
        
        if (size === 'small') {
            // 小：替換為 "No enlarged"
            textToCopy = textToCopy.replace(pattern, 'No enlarged');
        } else if (size === 'medium') {
            // 中：替換為 "Boarderline enlarged"
            textToCopy = textToCopy.replace(pattern, 'Boarderline enlarged');
        } else if (size === 'large') {
            // 大：替換為 "Enlarged"
                            ...
                            {hasEnlarged && !showEditButtons && (
                                <div className="flex items-center gap-[4px] ml-1">
                                    ...
                                    <button
                                        ...
                                        title="替換為 Boarderline enlarged"
                                        style={{ width: '20px', height: '20px' }}
                                    >
                                        中
                                    </button>
