function doPost(e) {
  try {
    ...
    tabs.forEach(tab => {
      ...
      // 分組版：左側 A=分組名 B=名稱 C=內容 / 右側 D=分組名 E=名稱 F=內容
      const headerRow = ['左側分組名', '左側組套名稱', '左側內容', '右側分組名', '右側組套名稱', '右側內容'];

      // 將分組與組套攤平為一列一列：[分組名, 名稱, 內容]
    ...
    return ContentService.createTextOutput(JSON.stringify({ status: 'success', message: '已成功更新所有頁籤（分組版 6 欄）' }))
      .setMimeType(ContentService.MimeType.JSON);
 ...
 * 新格式：A=左分組名 B=左名稱 C=左內容 D=右分組名 E=右名稱 F=右內容（左/右分組名預設為「預設」）
 */
function migrateAllSheetsToNewFormat() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const newHeader = ['左側分組名', '左側組套名稱', '左側內容', '右側分組名', '右側組套名稱', '右側內容'];
  ...
  ss.getSheets().forEach(sheet => {
    ...
    let newRows;
    if (maxCol <= 4) {
      // 舊 4 欄：A=左名稱 B=左內容 C=右名稱 D=右內容 → 插入分組名「預設」
      newRows = rows.map(row => [
        '預設',                    // A 左側分組名
        row[0] != null ? row[0] : '',  // B 左側名稱
        row[1] != null ? row[1] : '',  // C 左側內容
        '預設',                    // D 右側分組名
        row[2] != null ? row[2] : '',  // E 右側名稱
        row[3] != null ? row[3] : ''   // F 右側內容
      ]);
    } else {
      // 已是 6 欄：只確保表頭正確，資料不動
      const header = sheet.getRange(1, 1, 1, 6).getValues()[0];
      const isNewHeader = header[0] === '左側分組名' || header[0] === '左側組套名稱';
